...
mod verification {
    ...
    #[kani::requires(((n as u64 * (n as u64 + 1)) / 2) <= u8::MAX as u64)]
    #[kani::ensures(|result| *result as u64 == ((n as u64 * (n as u64 + 1)) / 2))]
    #[kani::recursion]
    fn triangle_rec(n: u8) -> u8 {
    ...
    fn check_triangle_rec() {
        let n: u8 = kani::any_where(|n| (*n as u64 * (*n as u64 + 1)) / 2 <= u8::MAX as u64);
        triangle_rec(n);
    }
    ...
    fn verify_triangle_rec() {
        ...
        assert_eq!(result, ((n as u64 * (n as u64 + 1)) / 2) as u8);
    }
}
